    ...
    try {
        ...
        if (new_ssh) {
            ...
            if (fs_1.default.existsSync(`${HOME_DIR}/.ssh/${name}.pub`)) {
                do {
                    name = await ui_1.ask_input(`${name} already exist, new Key name ?`);
            ...
            await utils_1.sh(`ssh-keygen -f ${HOME_DIR}/.ssh/${name} -N ""`);
            await utils_1.sh(`ssh-add ${HOME_DIR}/.ssh/${name}`);
            path = `${HOME_DIR}/.ssh/${name}.pub`;
        }
        else {
            path = await ui_1.ask_path('Ssh key path:', '\\.pub$', `${HOME_DIR}/`);
